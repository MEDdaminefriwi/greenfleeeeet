...
class _StopoverScreenState extends State<StopoverScreen> {
  ...
  void _addStopover() async {
    ...
    if (result != null && result is Map && result.containsKey("address")) {
      setState(() {
        stopovers.add(Map<String, dynamic>.from(result));
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    List<Widget> routeWidgets = [
      _buildLocationTile(widget.startAddress, Icons.location_on, isPrimary: true),
      ...stopovers
          .map((stop) => _buildLocationTile(stop['address'], Icons.stop_circle_outlined))
          .toList(),
      _buildLocationTile(widget.endAddress, Icons.flag, isPrimary: true),
    ];
